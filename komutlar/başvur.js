...
exports.run = async (client, message, args) => { 
  ...
  const sorular = [
    ...
    '**Neden Bizi Tercih Ediyorsunuz?** <cevabınız>'
  ]
  let sayac = 0
  ...
  collector.on('end', collected => {
    ...
    .addField('Başvuran Hakkında',[
      ...
      `**Neden Bizi Tercih Ettiniz?: **\t\t${collected.map(m => m.content).slice(5,6)}`
    ])
    .setTimestamp()
    .setFooter('Developed by HydroGENDegilim', message.guild.iconURL());
